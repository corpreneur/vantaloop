  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("intake_items").insert({
        title: subject.trim(),
        body: [goalOfShare, whatsWorking, questionsRisks, suggestions, decisionNeeded].filter(Boolean).join("\n\n") || null,
        submitter_name: name.trim(),
        source: "form" as const,
        status: "new" as const,
        feedback_type: feedbackType,
        goal_of_share: goalOfShare.trim() || null,
        whats_working: whatsWorking.trim() || null,
        questions_risks: questionsRisks.trim() || null,
      ...
      <footer className="border-t border-border mt-12">
        <div className="max-w-2xl mx-auto px-6 py-4">
          <p className="text-xs text-muted-foreground/40">
            VantaLoop -- Vanta Wireless × Metalab Design Feedback
          </p>
        </div>
      </footer>
    </div>
